...
class VGG11(nn.Module):
    """VGG11 full network (encoder + classifier head) per the official paper.

    Architecture: 8 conv layers in 5 blocks, each followed by MaxPool,
    then 3 fully-connected layers. BatchNorm added after each conv layer.
    CustomDropout added in the FC layers.

    Input image size is fixed at 224x224 as per the VGG paper.

    Args:
        num_classes: number of output classes (default 1000 for ImageNet,
                     use 37 for Oxford-IIIT Pet).
        in_channels: number of input image channels (default 3).
        dropout_p: dropout probability in FC layers (default 0.5).
    """
    ...
    def forward(
        ...
        Args:
            ...
            return_features: if True, return (bottleneck, skip_features) for
                             use in segmentation/multitask heads.

        Returns:
            - return_features=False: classification logits [B, num_classes].
            - return_features=True: (bottleneck [B,512,7,7], feature_dict)
        """
        features = {}
    ...
    def _block_with_skip(self, block: nn.Sequential, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        """Run block and capture pre-pool activations for skip connections."""
        pre_pool = x
        for layer in block:
...
# Alias so existing code using VGG11Encoder still works
VGG11Encoder = VGG11
